# 🛠 НАСТРОЙКА САЙТА — служебная инструкция (только для стримера)

Этот файл — твоя личная памятка по настройке. Зрителям достаточно `README.md`.
На сам сайт этот файл никак не влияет.
...
| `README.md` | Гайд «как отправить видео» | зрители |
| `SETUP.md` | Эта инструкция | ты |
...
⚠️ **databaseURL обязательно проверь** — параметра может не быть в блоке из консоли. Без него уведомления работать не будут.
...
3. **Add file → Upload files** → перетащи ВСЕ файлы сайта → **Commit changes**.
...
Карточки **не исчезают сами**: нажми **✕** (скип) или **«Смотреть видео»** — и карточка уйдёт с экрана.

Полезно: правый клик по источнику → **Interact** — можно кликать по кнопкам карточки прямо из OBS (видео откроется в браузере).
...
| `duration` | `0` = карточка висит, пока не скипнешь (✕) или не нажмёшь «Смотреть видео»; число = авто-скрытие через столько секунд |
...
widget.html?dur=0       — карточка висит, пока не скипнешь (по умолчанию)
widget.html?dur=14      — карточка сама исчезнет через 14 секунд
...
| Красная ошибка «permission denied» | Тестовый режим истёк или правила режут доступ — вставь правила из шага 6 |
